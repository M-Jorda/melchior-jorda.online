...
const pseudoChecker = (value) => {
    ...
    // const errorDisplay = document.querySelector('.pseudo-container > span');
     ...
     if (value.length > 0 && (value.length < 3 || value.length > 20)) {
        ...
        //  errorDisplay.textContent = "Le pseudo doit faire entre 3 et 20 caracteres.";
        errorDisplay('pseudo', 'Le pseudo doit faire entre 3 et 20 caracteres');
        pseudo = null;
     } else if (!value.match(/^[a-zA-Z0-9_.-]*$/)) {
        //  pseudoContainer.classList.add('error');
        //  errorDisplay.textContent = "Le pseudo ne doit pas contenir de caracteres speciaux.";
...
const passwordChecker = (value) => {
    ...
    if (!value.match(/^(?=.*?[A-Z])(?=(.*[a-z]){1,})(?=(.*[\d]){1,})(?=(.*[\W]){1,})(?!.*\s).{8,}$/)) {
        errorDisplay("password", "minimum de 8 caracteres, une maj., un chiffre et un caractere special.");
...
form.addEventListener('submit', (e) => {
    ...
    if (pseudo && email && password && confirmPass) {
        ...
        alert('veuillex remplir les champs.');
    }
});